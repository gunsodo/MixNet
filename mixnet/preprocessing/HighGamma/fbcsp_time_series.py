...
def subject_dependent_setting(k_folds, pick_smp_freq, n_components, n_features, bands, order, save_path, num_class=2, sel_chs=None):
    ...
    n_folds = k_folds
    save_path = save_path + '/HighGamma/fbcsp_time_series/{}_class/{}_csp_components/subject_dependent'.format(num_class,n_components)
    n_chs = len(sel_chs)

    X_train_all, y_train_all = [], []
    X_test_all, y_test_all = [], []
    print("The number of CSP component used is: ", n_components, 'with using ',num_class, ' classes data and preparing on dataset: ', 'HighGamma')
...
def subject_independent_setting(k_folds, pick_smp_freq, n_components, n_features, bands, order, save_path, num_class=2, sel_chs=None):
    sel_chs = CONSTANT['sel_chs'] if sel_chs == None else sel_chs
    n_folds = k_folds
    save_path = save_path + '/HighGamma/fbcsp_time_series/{}_class/{}_csp_components/subject_independent'.format(num_class,n_components)
    n_chs = len(sel_chs)
