...
def multi_bands_subject_independent_setting(k_folds, pick_smp_freq, bands, order, save_path, num_class=2, sel_chs=None):
    sel_chs = CONSTANT['sel_chs'] if sel_chs == None else sel_chs
    n_folds = k_folds
    save_path = save_path + '/SMR_BCI/time_domain_multi_bands/{}_class/subject_independent'.format(num_class)
    ...
    for person, (X_val, y_val, X_te, y_te) in enumerate(zip(X_train_all, y_train_all, X_test_all, y_test_all)):
        ...
        for fold, (train_ind, val_ind) in enumerate(skf.split(fake_tr , fake_tr_la)):
            ...
            # Performing bandpass-filtering
            X_train_fil, X_val_fil, X_test_fil = [], [], []
            for band in bands:
                print('Band-pass filtering from {} to {} Hz.'.format(band[0],  band[1]))
                X_tr_fil =  butter_bandpass_filter(X_train, band[0],  band[1], pick_smp_freq, order)
                X_va_fil = butter_bandpass_filter(X_val, band[0],  band[1], pick_smp_freq, order)
                X_te_fil = butter_bandpass_filter(X_test, band[0],  band[1], pick_smp_freq, order)
                X_train_fil.append(X_tr_fil)
                X_val_fil.append(X_va_fil)
                X_test_fil.append(X_te_fil)
            X_train_fil = np.array(X_train_fil).swapaxes(0,1)
            X_val_fil = np.array(X_val_fil).swapaxes(0,1)
            X_test_fil = np.array(X_test_fil).swapaxes(0,1)
